...
class lx_cr_session implements phpCR_Session {
	 ...
	 * 
	 */
	function __construct($credentials, $workspacename, $storage) {
		$this->storage = $storage;
		$this->workspace = new lx_cr_workspace($workspacename,$this);
...
If another error occurs 
	...
	public function getNodeByUUID($uuid) {
		...
		$node = $this->getFromNodesList($uuid);
        if ($node) {
            return $node;
        }
        $node = new lx_cr_node($this);
        $vals = $this->storage->getNodeByUUID($uuid);
        if (is_array($vals)) {
            $node->init($vals);
...
A {@link Repository} object 
	 ...
	 * workspace.
	 *
	 * @return lx_cr_node The root node of the workspace: a {@link Node} object.
	 *
	 * @throws {@link RepositoryException}
	 *    If an error occurs.
	 */
	...
	public function getRootNode() {
		if ($this->rootNode === null) {
			$this->rootNode = new lx_cr_node($this);
			$this->rootNode->init($this->storage->getRootNodeValues());
			$this->nodes[$this->rootNode->getUUID()] = $this->rootNode;
...
If another error occurs. 
	...
	public function addNodeToModifiedList($node) {
		...
		$this->modifiedNodes[$node->getUUID()] = $node;
	}
	
	public function getFromNodesList($uuid) {
	 ...
	 public function parsePath($path, lx_cr_node $node,$noAbsolute = false) {
	...
	protected function getAllDescendants(lx_cr_node $node) {
		$nodes = $node->getNodes();
